...
class Piece():
    ...
    def move_piece(self, y, x):
        ...
        self.img.move((x * 100 + 50) - (self.x * 100 + 50),
                      (y * 100 + 50) - (self.y * 100 + 50))
...
class Pawn(Piece):
        ...
        if self.color == 'white' and self.y == 3:
            for i in range(-1, 2):
                ...
                if (
                    ...
                    moves.append(
                        Move(self.y-1, self.x+i, self.win, 'en_passant'))
                    break
            ...
            for i in range(-1, 2):
                ...
                if (
                    ...
                    moves.append(Move(self.y + 1, self.x +
                                 i, self.win, 'en_passant'))
                    break
    ...
    def promotion(self, board):
        ...
        for i in range(4):
            prom_pieces[i] = prom_pieces[i](
                color, (y_mid_px - 50) / 100, (start_x + (i * 100)) / 100, win)
        ...
        def chosen_piece():
            ...
            while True:  # wait for input
                ...
                if (
                    clickPoint != None and
                    int(clickPoint.y) in range(
                        int(y_mid_px - size_y/2), int(y_mid_px + size_y/2))
                ):
                    for i in range(4):
            ...
            for x in range(-2, 3):
                if (
                    ...
                    if board[self.y + y][self.x + x] == None:
                        ...
                        moves.append(
                            Move(self.y + y, self.x + x, self.win, 'takes'))
...
class Queen(Bishop, Rook):
    ...
    def all_moves(self, board, check_king_safety=True):
        ...
        """
        moves = []
        moves += Bishop.bishop_moves(self, board) + \
            Rook.rook_moves(self, board)
...
class King(Piece):
    ...
    def all_moves(self, board, check_king_safety=True):
        ...
        moves += self.king_moves(board) + \
            self.castle_short(board) + self.castle_long(board)
